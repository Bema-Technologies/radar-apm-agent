...
export async function wrapWebApp() {
  ...
  WebApp.rawConnectHandlers.stack.unshift({
    route: '',
    handle: (req, res, next) => {
      req._radar_id = Random.id();

      const name = parseUrl(req).pathname;
      const trace = Kadira.tracer.start({ id: req._radar_id }, {
        id: req._radar_id,
        msg: 'http',
      ...
      Kadira.tracer.event(trace, 'start', {
        url: req.url,
        method: req.method
        // headers: req.headers || {}, // hidden for now
      });
      ...
      res.on('finish', () => {
        ...
        if (req.__kadiraInfo.isStatic) {
          trace.name = `[${req.method}]<static_file>`
          ...
          trace.name = `[${req.method}]<app_route>`
        }
        ...
        */
        ...
        Kadira.models.webapp.processRequest(built, req, res);
      });
