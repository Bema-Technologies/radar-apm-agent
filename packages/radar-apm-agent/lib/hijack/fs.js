...
export function wrapFs() {
  ...
  fs.stat = function () {
    ...
    if (kadiraInfo) {
      let eventId = Kadira.tracer.event(kadiraInfo.trace, 'fs', {
        func: 'stat',
        path: arguments[0],
  ...
  fs.createReadStream = function () {
    ...
    if (kadiraInfo) {
      const eventId = Kadira.tracer.event(kadiraInfo.trace, 'fs', {
      ...
      handleErrorEvent(stream, kadiraInfo.trace, eventId);
    }
