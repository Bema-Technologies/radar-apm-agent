...
MethodsModel.prototype._appendMetrics = function (id, methodTrace) {
  ...
  if (methodTrace.metrics.total > methodMetrics.maxResTime) {
    methodMetrics.maxResTime = methodTrace.metrics.total;
  }
...
MethodsModel.prototype.buildPayload = function (buildDetailedInfo) {
  ...
  let methodRequests = this.tracerStore.collectTraces();
...
MethodsModel.prototype.addIgnoredMethod = function (methods) {
  if (Array.isArray(methods)) {
    methods.forEach((method) => {
      this.ignoredMethods.add(method)
    })
  } else {
    this.ignoredMethods.add(methods);
  }
}
